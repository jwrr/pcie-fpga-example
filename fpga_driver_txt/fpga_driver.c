...
static u32    fpga_rdata32[512] = {0};
static short  fpga_rdata32_size_in_bytes;
...
static ssize_t dev_write(struct file *filep, const char *buffer, size_t len, loff_t *offset)
{
  ...
  sscanf(buffer, "%c %x %x", &cmd, &byte_addr, &data);
  
  printk(KERN_INFO "fpga_driver.dev_write - cmd=%c byte_addr=0x%x data=0x%x\n", cmd, byte_addr, data);
  
  if (cmd == 'w') {
    iowrite32(data,ptr_bar0+byte_addr);
  } else {
    fpga_rdata32[0] = ioread32(ptr_bar0+byte_addr);
    fpga_rdata32_size_in_bytes = 4;
  }
